...
public class Find_Search_V1_Refinement_Step2 implements Serializable {
	...
	@GET
	@Path("/Refinement_Step2")
	@Produces("application/json,application/xml")
	@ApiOperation(value = " In each refinement button embedded curl queries", notes = " In each refinement button embedded curl queries,"
			+ "  Add keyword from each  refinement button to main keyword (server34 + refinement button keword )") // Specific
	...
	public Response getByID(@ApiParam(value = "query 1 keywords type: String", required = true) @DefaultValue("") @QueryParam("query1") String query1,
		...
		String json = Find_Search_V1_Refinement_Step2.curlToJsonString(query1, query2);
		//System.out.println(json);
		
		try {
			postResponse = Find_Search_V1_Refinement_Step2.doPostRequest("http://192.168.23.10:9200/it_com_goods_20180919/_search", json);
