...
public class Find_Search_V1_Sorting implements Serializable {
	...
	@ApiOperation(value = "sort search result by specific sort parameters (asc,desc).", notes = "result contains user's query in \"product_name\" or \"seller_name\" or \"product_description\" or \"product_variant\" or \"categories_name.lv1 - categories_name.lv7\" fields AND results must order by sort parameters (asc,desc)") // Specific
	...
	public Response getByID(@ApiParam(value = "query 1 keywords type: String", required = true) @DefaultValue("") @QueryParam("query1") String query1,
			...
			@ApiParam(value = "Sorting method type: String", required = true) @DefaultValue("product_rating") @QueryParam("sortMethod") String sortMethod,
			@ApiParam(value = "Sorting order type: String", required = true) @DefaultValue("desc") @QueryParam("sortOrder") String sortOrder) {

		
		
		String postResponse = null;
		String json = Find_Search_V1_Sorting.curlToJsonString(query1, query2, sortMethod, sortOrder);
		//System.out.println(json);
		
		try {
			postResponse = Find_Search_V1_Sorting.doPostRequest("http://192.168.23.10:9200/it_com_goods_20180919/_search", json);
	...
	static String curlToJsonString(String query1, String query2, String sortMethod, String sortOrder) {
		...
		return	"{\"query\":{\"bool\":{\"should\":[{\"multi_match\":{\"minimum_should_match\":\"75%\",\"type\":\"cross_fields\","
				...
				+ "\"minimum_should_match\":1}},\"size\":1000,\"sort\":[{\""+sortMethod+"\":{\"order\":\""+ sortOrder+"\"}}]}";	
