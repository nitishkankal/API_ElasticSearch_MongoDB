...
package com.ApiServices.Elasticsearch.Find_Search_V1;
...
@Path("/Find_Search_V1")
@Api(value = "Find_Search_V1")
public class Find_Search_V1_FilterByPrice implements Serializable {
	...
	@ApiOperation(value = " users filter product by product price.  ", notes = "Logic : we should provide price range button (ex.1000 - 2000 , 3000 - 4000) for user , "
			+ "results must come from \"product_price.num\" field AND contains user's query in \"product_name\" or \"seller_name\" or \"product_description\" or"
			+ " \"product_variant\" or \"categories_name.lv1 - categories_name.lv7\" fields.") // Specific
	...
	public Response getByID(@ApiParam(value = "query 1 keywords type: String", required = true) @DefaultValue("") @QueryParam("query1") String query1,
			...
			@ApiParam(value = "Product price key type: String", required = true) @DefaultValue("product_price.num") @QueryParam("productPriceKey") String key,
			@ApiParam(value = "price min value type: Integer", required = true) @DefaultValue("") @QueryParam("minPrice") Integer value,
			@ApiParam(value = "price max value type: Integer", required = true) @DefaultValue("") @QueryParam("maxPrice") Integer value1) {

		
		
		String postResponse = null;
		String json = Find_Search_V1_FilterByPrice.curlToJsonString(query1, query2, key, value,value1);
		//System.out.println(json);
		
		try {
			postResponse = Find_Search_V1_FilterByPrice.doPostRequest("http://192.168.23.10:9200/it_com_goods_20180919/_search", json);
	...
	static String curlToJsonString(String query1, String query2, String key, Integer value, Integer value1) {
		...
		return	"{\"query\":{\"bool\":{\"should\":[{\"multi_match\":{\"minimum_should_match\":\"25%\",\"type\":\"cross_fields\","
				...
				+"\"filter\":{\"range\":{\""+key+"\":{\"gte\":"+value+",\"lte\":"+value1+"}}},\"minimum_should_match\":1}},\"size\":1000}";
