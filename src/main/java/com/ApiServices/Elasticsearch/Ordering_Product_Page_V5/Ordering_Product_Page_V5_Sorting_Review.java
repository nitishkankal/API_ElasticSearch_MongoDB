...
public class Ordering_Product_Page_V5_Sorting_Review implements Serializable {
	...
	@ApiOperation(value = " users sort review on specific product.   ", notes = "Logic : results must order by sort parameters (asc,desc) ex. review_type,review_date,review_rating") // Specific
	...
	public Response getByID(@ApiParam(value = "Product ID  type: String", required = true) @DefaultValue("") @QueryParam("query1") String query1,
			@ApiParam(value = "Sorting method type: String", required = true) @DefaultValue("review_date") @QueryParam("query2") String query2, 
			@ApiParam(value = "Sort Order - asc/desc: String", required = true) @DefaultValue("") @QueryParam("filter_Keyword") String value) {

		
		
		String postResponse = null;
		String json = Ordering_Product_Page_V5_Sorting_Review.curlToJsonString(query1, query2 , value);
		System.out.println(json);
		
		try {
			postResponse = Ordering_Product_Page_V5_Sorting_Review.doPostRequest("http://192.168.23.10:9200/product_review_20180919/_search", json);
	...
	static String curlToJsonString(String query1, String query2,  String value) {
		...
		return  "{\"query\":{\"bool\":{\"filter\":{\"term\":{\"product_id\":"+query1+"}}}},\"sort\":[{\""+query2+"\":{\"order\":\""+value+"\"}}],\"size\":1000}";
