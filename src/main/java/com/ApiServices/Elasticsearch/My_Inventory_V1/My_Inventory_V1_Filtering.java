...
public class My_Inventory_V1_Filtering implements Serializable {
	...
	@ApiOperation(value = " users search for conpon first then apply filter parameters. ", notes = "  result contains user's query in \"coupon_name\" or \"seller_name\" fields AND all result must come from \"Categories level 2.keyword\" field.") // Specific
	...
	public Response getByID(@ApiParam(value = "query 1 keywords type: String", required = true) @DefaultValue("") @QueryParam("query1") String query1,
		...
		String json = My_Inventory_V1_Filtering.curlToJsonString(query1, query2, key, value);
		//System.out.println(json);
		
		try {
			postResponse = My_Inventory_V1_Filtering.doPostRequest("http://192.168.23.10:9200/coupon_20180919/_search", json);
