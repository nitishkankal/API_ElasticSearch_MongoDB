...
public class Prevent_0_Result_Step_2 implements Serializable {
	...
	@ApiOperation(value = "When search API got 0 result , Go to check keyword suggestion API", notes = "Logic :Keyword Suggest API : If system detect 0 result from search api , use this api to use top suggest keyword for put as search keyword instead user original keyword. ") // Specific
	...
	public Response getByID(@ApiParam(value = "Search keyword type: String", required = true) @DefaultValue("") @QueryParam("query1") String query1) {
		...
		String json = Prevent_0_Result_Step_2.curlToJsonString(query1);
		//System.out.println(json);
		
		try {
			postResponse = Prevent_0_Result_Step_2.doPostRequest("http://192.168.23.10:9200/it_com_goods_20180919/_search", json);
