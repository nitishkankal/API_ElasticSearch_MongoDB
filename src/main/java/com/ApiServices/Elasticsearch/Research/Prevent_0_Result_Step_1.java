...
public class Prevent_0_Result_Step_1 implements Serializable {
	...
	@ApiOperation(value = "Search by keyword (ex.sam) but got 0 result ", notes = "Logic :\r\n" + 
			"1.Search API : user type keyword in search box normally.\r\n" + 
			"\r\n" + 
			"2.Keyword Suggest API : If system detect 0 result from search api , use this api to use top suggest keyword for put as search keyword instead user original keyword.\r\n" + 
			"\r\n" + 
			"3.Search API : Use keyword from Keyword Suggest API and send this result to users.") // Specific
	...
	public Response getByID(@ApiParam(value = "Search  keyword type: String", required = true) @DefaultValue("") @QueryParam("query1") String query1) {
		...
		String json = Prevent_0_Result_Step_1.curlToJsonString(query1);
		//System.out.println(json);
		
		try {
			postResponse = Prevent_0_Result_Step_1.doPostRequest("http://192.168.23.10:9200/it_com_goods_20180919/_search", json);
